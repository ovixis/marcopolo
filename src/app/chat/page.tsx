      ...
      <div className="flex w-80 shrink-0 flex-col border-r border-border">
        ...
        <div className="flex flex-col gap-3 border-t border-border p-5">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            Your model
            <span
              className={`ml-auto size-2 rounded-full ${connected ? "bg-emerald-400" : "bg-muted-foreground/40"}`}
              ...
              <div className="mt-8 flex flex-col gap-2">
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
                    onClick={() => send(s)}
                    className="rounded-xl border border-border bg-card px-4 py-3 text-left text-sm text-muted-foreground transition-colors hover:border-primary/40 hover:bg-primary/5"
                  >
                    {s}
                  </button>
                ))}
